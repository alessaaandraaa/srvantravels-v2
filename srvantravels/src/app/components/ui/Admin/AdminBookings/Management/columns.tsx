...
import { Check, ChevronDown, CreditCard, MapPin, MoreHorizontal } from "lucide-react"; 
import {
  Command,
  CommandEmpty,
  CommandGroup,
...
export const getColumns = (drivers: Driver[], vans: Van[]): ColumnDef<Order>[] => [
  {
            ...
            <div className="max-w-[150px]">
                <Popover open={open} onOpenChange={setOpen}>
                    <PopoverTrigger asChild>
                        ...
                        className={cn(`w-full justify-between`, value ? "text-green-300" : "text-destructive")}  
                        >
                        {value
                            ? (() => {
                                const driver = drivers.find((d) => d.name === value)
                                if (!driver) return "Unassigned"
                                return driver.name.length > 8 ? `${driver.name.substring(0,8)}...` : driver.name
                            })()
                            : "Unassigned"}
                                    ...
                                    <CommandGroup className="flex flex-col gap-4">
                                        {drivers.map((drivers) => (
                                            ...
                                            <Check
                                                className={cn(
                                                "ml-auto",
                                                value === drivers.name ? "opacity-100" : "opacity-0"
                                                )}
                                            />
                                            </CommandItem>
                                        ))}
            ...
            <div className="max-w-[150px]">
                <Popover open={open} onOpenChange={setOpen}>
                    <PopoverTrigger asChild>
                        ...
                        {value
                            ? (() => {
                                const driver = vans.find((d) => d.vanplatenumber === value)
                                if (!driver) return "Unassigned"
                                return driver.vanplatenumber.length > 8 ? `${driver.vanplatenumber.substring(0,8)}...` : driver.vanplatenumber
                            })()
                            : "Unassigned"}
                        <ChevronDown className="opacity-50 flex-shrink-0" />
                        </Button>
                    ...
                    <PopoverContent className="w-full p-0">
                        <Command filter={(value, search) => {
                                ...
                                return van && (String(van.capacity).includes(search.toLowerCase())) ? 1 : 0
                            }}>
                                    ...
                                    <CommandGroup className="flex flex-col gap-4">
                                        {vans.map((vans) => (   
                                            ...
                                            <Check
                                                className={cn(
                                                "ml-auto",
                                                value === vans.vanplatenumber ? "opacity-100" : "opacity-0"
                                                )}
                                            />
                                            </CommandItem>
                                        ))}
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem onClick={() => navigator.clipboard.writeText(order.orderid.toString())}>
                        Copy Order ID
                    </DropdownMenuItem> 
            ...
            <DialogContent className="w-[88vw] h-[80vh] max-w-none rounded-md overflow-y-auto">
                <div className="flex flex-col gap-4 rounded-lg">
                    <div className="mt-6 flex flex-col gap-1">
                        <div className="bg-primary-foreground w-auto h-[120px] rounded-lg">
                        </div>
                        {/* 
